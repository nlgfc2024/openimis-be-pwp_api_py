...
with ZipFile(wheel) as archive:
    ...
    assert "pwp_api/schema.py" in names
    assert not any(f"pwp_api/{module}.py" in names
                   for module in ("views", "serializers", "authentication", "schema_urls"))
    ...
    for dependency in ("insuree", "claim", "policy", "policyholder",
                       "fhir.resources", "djangorestframework", "drf-spectacular"):
